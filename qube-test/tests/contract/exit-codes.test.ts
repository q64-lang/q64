...
describe.skipIf(!binaryAvailable())("exit codes (spec surface)", () => {
  test.skipIf(!q64Available())("a clean `build` exits 0", () => {
    const proj = makeProject({ "qube.json5": appManifest(), "src/main.q": "fn main { env.out(\"x\") }\n" });
    expect(runCli(["build", "--addr", "wasm64"], { cwd: proj, env: { Q64_BIN } }).exitCode).toBe(0);
  });

  test.skipIf(!q64Available())("a compile error from q64 exits 64", () => {
    const proj = makeProject({
      "qube.json5": appManifest(),
      "src/main.q": "import q64.math.*\nfn main { env.out(\"x\") }\n",
    });
    expect(runCli(["build", "--addr", "wasm64"], { cwd: proj, env: { Q64_BIN } }).exitCode).toBe(64);
  });
  ...
  test("a dependency cache miss with --offline exits 66", () => {
    ...
    expect(runCli(["build", "--offline", "--addr", "wasm64"], { cwd: proj, timeout: 15_000 }).exitCode).toBe(66);
  });
});
