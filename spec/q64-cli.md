...
| `q64 show symbols <file.q>`     | The sema pass's file-level symbol table: one line per top-level item and per import binding (kind, name, visibility, import origin), plus recorded name collisions. `fit` declarations are listed but are not name bindings. Compiler-introspection (unstable format); see `q64/src/sema/README.md`. |
