...
The source qube is given positionally for `hir`/`mir`/`symbols` (`q64 show hir
...
| `--component`                     | Also wrap the core module in a WebAssembly component (per [`modules.md` §"The qube as a component"](./modules.md)). Writes `<out>.component.wasm` **and** `<out>.wit` (the synthesized world, same as [`q64 show world`](#show)) *in addition to* the core `--out` module — the core module is still produced. Set by `qube build --component`. The component embeds its own type, so `wasm-tools component wit <out>.component.wasm` round-trips the world with the real export/parameter names (kebab-cased from q64's snake_case); `<out>.wit` is its source-level companion (the contract the Continuum stores and `wac` consumes). **v0 coverage:** lifts an import-free core module's scalar export surface (`s64`/`bool`/`f64`); and, on `--addr wasm32`, emits an app as a WASI **preview1** core (its `env.out` lowered to a `wasi_snapshot_preview1.fd_write` import) and runs `wasm-tools component new --adapt` with the vendored WASI adapter to lift it into a real `wasi:cli/run` command importing `wasi:cli/stdout` — the world [`q64 show world`](#show) names. `wasm-tools` + the adapter are located via `Q64_WASM_TOOLS` / `Q64_WASI_ADAPTER` (else the repo `vendor/`, else `PATH`). `str`/list exports and the wasm64 import ABI are skipped pending the canonical-ABI memory glue; a wasm64 capability-importing qube errors (`ComponentNeedsImportLowering`). |
| `--world <name>`                  | Name of the synthesized WIT world for `--component` / `show world` (WIT rung 2). Default: the source-file basename. `qube build --component` passes the manifest's `wit.world` (see [`qube.json5.md` §WIT](./qube.json5.md)) so the emitted `<out>.wit` carries the qube's identity, not the entry filename. |
| `--wit-package <id>`              | WIT package id (`namespace:name`) for the synthesized world. Default: `q64:<world>`. `qube build` passes the manifest's `wit.package` (default derived from the qube `name`). |
| `--wit-import <file.wit>`          | Declare a foreign WIT package this qube **imports** in the emitted component's world (WIT rung 5 — the codegen import binding; what `wac` links at build). Repeatable. Only the **scalar**-signature functions of each interface are declared (the same canonical-ABI boundary as scalar exports; non-scalar funcs are skipped). `qube build` passes the manifest's `wit.imports`. The component-level import is a forward declaration; the source-level *call* (q64 code invoking it) is the next slice. |
