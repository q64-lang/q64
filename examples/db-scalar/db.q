...
//! + `query_text` (as text). Typed multi-row `Rows`, positional params, and
//! `batch` are deferred (they need a variable-length typed decode q64 does not
//! yet emit); v0 takes literal SQL. env.db targets a q64-owned `q64:db/sql`,
//! not raw wasi:sql — see the WIT header and test/db-component-reference/.
