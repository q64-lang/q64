...
> `let`/pattern bindings); functions hover with their full signature; the file
> outline (document symbols) and completion (symbols + the enclosing function's
> locals + keywords) are served too. Still pending: field/member hover &
> completion (the type checker) and formatting (`fmt`) in [`../q64`](../q64).
