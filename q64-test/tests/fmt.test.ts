 ...
 *
 * Covers the v0 formatter (a structural reindenter): in-place rewrite,
 * stdin/stdout, and the `--check` / `--lint` modes.
 */
...
describe.skipIf(!binaryAvailable())("q64 fmt", () => {
  test("`fmt path` formats a file in place", () => {
  ...
  test("`fmt --stdout` reads stdin and writes the formatted result to stdout", () => {
    const r = runCli(["fmt", "--stdout"], { stdin: "fn   main{\n env.out( \"x\" ) }\n" });
    expect(r.exitCode).toBe(0);
    expect(r.stdout).toContain("fn main");
  });

  test("`fmt --check` exits 64 when a file would be reformatted", () => {
    expect(runCli(["fmt", "--check", fixture("unformatted.q")]).exitCode).toBe(64);
  });

  test("`fmt --check` exits 0 when already formatted", () => {
    expect(runCli(["fmt", "--check", fixture("hello.q")]).exitCode).toBe(0);
  });

  test("`fmt --lint` reports formatting issues on stderr without modifying files", () => {
