...
pub const codes = [_]CodeInfo{
    ...
    .{ .code = "PAR050", .subsystem = "Parser", .severity = .err, .message = "statements must be separated by a newline or `;`", .summary = "A second statement follows another on the same line with nothing between them (e.g. `a = 1  b = 2`). q64 ends a statement at a newline, a `;`, or `}`, so the trailing tokens would be silently dropped. Put the next statement on its own line, or separate them with `;` (`a = 1; b = 2`)." },
    // Names
    ...
    .{ .code = "TYP047", .subsystem = "Type checking", .severity = .err, .message = "optional type not narrowed", .summary = "A value of type `T?` is used as a `T` without first narrowing it. Narrow with `if let Some(x) = opt { … }`, `match`, or chain with `?.`; the `T?` only becomes `T` on the branch where it's known present. See spec/types.md §\"optional types and flow narrowing\"." },
    ...
    .{ .code = "TYP306", .subsystem = "Type checking", .severity = .err, .message = "`panic` payload does not fit `Panic`", .summary = "`panic <expr>` requires the payload's type to fit the `Panic` face. Add `fit YourType : Panic { … }`, use a string (`panic \"…\"`) or a blessed prelude payload (`Cancelled`, `Closed`, `RuntimeDenied`). See spec/errors.md §\"`panic` and `trap`\"." },
    .{ .code = "TYP350", .subsystem = "Type checking", .severity = .err, .message = "lambda needs an expected function type", .summary = "A lambda (`|x| …`) appears where no `fn` type is expected (e.g. an unannotated `let`). Annotate the binding (`let f: fn(i64) -> i64 = |x| …`) or pass it where a function is expected; v0 has no standalone inference of lambda parameter types. See spec/closures.md." },
    .{ .code = "TYP351", .subsystem = "Type checking", .severity = .err, .message = "lambda arity mismatch", .summary = "A lambda's parameter count differs from the expected `fn` type's. Match the parameter count (`fn(i64, i64) -> _` expects `|a, b| …`). See spec/closures.md." },
    .{ .code = "TYP360", .subsystem = "Type checking", .severity = .err, .message = "no operator fit for this type", .summary = "An operator is used on a type with no fit for its face — `a + b` desugars to `a.add(b)` and needs `fit Type : Add` (`-` Sub, `*` Mul, `/` Div, `%` Rem, unary `-` Neg). Declare the fit, or write the method call you mean. See spec/operators.md." },
    .{ .code = "TYP361", .subsystem = "Type checking", .severity = .err, .message = "operand type mismatch in operator expression", .summary = "Operator fits are homogeneous in v0 (`Self × Self → Self`), and q64 never converts implicitly — both operands must be the same type. Convert one side explicitly (e.g. construct the struct from the scalar) before applying the operator. See spec/operators.md §Rules." },
    .{ .code = "TYP207", .subsystem = "Type checking", .severity = .err, .message = "face is not dyn-safe", .summary = "`dyn Face` requires every method to take `self`, return something other than `Self`, and carry no method-level type parameters (`Clone.clone -> Self` precludes it). Use a generic bound `<T: Face>` instead, or pick a dyn-safe face. See spec/faces.md §\"dyn-safety\"." },
    ...
    .{ .code = "TYP102", .subsystem = "Type checking", .severity = .err, .message = "missing required generic argument", .summary = "A generic parameter has no default and can't be inferred from the arguments (e.g. it appears only in the return type). Supply it explicitly — `buffer<i64>()` — or give the parameter a default. See spec/generics.md §\"inference then default\"." },
    ...
    .{ .code = "EFF100", .subsystem = "Effects", .severity = .err, .message = "`panic` in a `@no_panic` function", .summary = "The function is declared (or implies, via `@realtime`/`@pure`/`@no_alloc`) `@no_panic`, but its body contains a `panic`. A panic unwinds and its payload allocates — neither is audio-thread safe. Return a `Result`, or `trap()` for an invariant violation (traps are permitted under `@no_panic`). See spec/effects.md §\"What `@no_alloc` and `@no_suspend` forbid\"." },
    .{ .code = "EFF101", .subsystem = "Effects", .severity = .err, .message = "`trap` in a `@no_trap` function", .summary = "The function is declared (or implies, via `@pure`) `@no_trap`, but its body calls `trap()`. Handle the condition with a `Result` instead. See spec/effects.md §\"The assert markers\"." },
    .{ .code = "EFF110", .subsystem = "Effects", .severity = .err, .message = "callee effect outside caller's set", .summary = "The body uses a capability the function's declared effect set forbids — e.g. an `@io`-bearing `env` face or callee inside a `@realtime` function. `@realtime` permits only the realtime-safe `@time`/`@random`/`@audio` surfaces. Move the operation off the real-time path, or widen the declaration. See spec/effects.md §\"Propagation through call graphs\"." },
    .{ .code = "EFF112", .subsystem = "Effects", .severity = .err, .message = "assert violated by callee or operation", .summary = "An operation in (or reached from) this function violates one of its declared asserts: an allocation under `@no_alloc` (`Vec.new`/`Vec.from`, `push` copy-on-grow, string interpolation, `channel`), a suspension under `@no_suspend` (`spawn`, `env.sleep_ns`, `Managed.box`), or a callee that doesn't satisfy the caller's asserts. Pre-allocate outside the asserted function, or annotate the callee. See spec/effects.md §\"What `@no_alloc` and `@no_suspend` forbid\"." },
    ...
    .{ .code = "STR020", .subsystem = "Streams", .severity = .err, .message = "dataflow type mismatch in pipeline", .summary = "A stage was given the wrong dataflow type — e.g. an `Event<T>` where a `Signal<T>` is expected (or vice-versa). Events are pointwise; signals are continuous. Convert explicitly (`hold`, `sample`) or fix the stage's input type. See spec/streams.md §\"Why three types instead of one\"." },
