...
// Build stamp + snap config — OPTIONAL artifacts (a gitignored version.js, a
// deploy-filled snap-config.js) that may be absent. Load them NON-BLOCKING, never
// via top-level `await`: a top-level `await import()` of a missing module (or one
// the host serves as an HTML 404/SPA fallback) can abort the entire module load —
// observed on the deployed host page, where app.js silently never executed (blank
// page, no logs). The values are cosmetic (header stamp, snap button) and read
// later in main(), so a slightly-late background update is fine.
let BUILD = 'dev';
let SNAP = { api: '', token: '' };
import('./version.js').then((m) => { if (m && m.BUILD) BUILD = m.BUILD; }).catch(() => {});
import('./snap-config.js').then((m) => { if (m && m.SNAP) SNAP = m.SNAP; }).catch(() => {});
